...
impl Element {
    pub fn new_inherited(type_id: ElementTypeId, local_name: DOMString, namespace: Atom, prefix: Option<DOMString>, document: JSRef<Document>) -> Element {
        Element {
    ...
    pub fn new(local_name: DOMString, namespace: Atom, prefix: Option<DOMString>, document: JSRef<Document>) -> Temporary<Element> {
...
pub trait RawLayoutElementHelpers {
    unsafe fn get_attr_val_for_layout(&self, namespace: &Atom, name: &str) -> Option<&'static str>;
    unsafe fn get_attr_atom_for_layout(&self, namespace: &Atom, name: &str) -> Option<Atom>;
...
impl RawLayoutElementHelpers for Element {
    ...
    unsafe fn get_attr_val_for_layout(&self, namespace: &Atom, name: &str)
                                      -> Option<&'static str> {
    ...
    #[inline]
    #[allow(unrooted_must_root)]
    unsafe fn get_attr_atom_for_layout(&self, namespace: &Atom, name: &str)
                                      -> Option<Atom> {
        // cast to point to T in RefCell<T> directly
        let attrs: *const Vec<JS<Attr>> = mem::transmute(&self.attrs);
        (*attrs).iter().find(|attr: & &JS<Attr>| {
...
impl LayoutElementHelpers for JS<Element> {
    #[allow(unrooted_must_root)]
    unsafe fn html_element_in_html_document_for_layout(&self) -> bool {
        if (*self.unsafe_get()).namespace != sns!(HTML) {
            return false
        }
...
impl<'a> ElementHelpers for JSRef<'a, Element> {
    fn html_element_in_html_document(&self) -> bool {
        ...
        self.namespace == sns!(HTML) && node.is_in_html_doc()
    }
...
pub trait AttributeHandlers {
    ...
    /// name, if any.
    fn get_attribute(self, namespace: Atom, local_name: &str)
    ...
    fn parse_attribute(self, namespace: &Atom, local_name: &Atom,
                       value: DOMString) -> AttrValue;

    fn remove_attribute(self, namespace: Atom, name: &str);
...
impl<'a> AttributeHandlers for JSRef<'a, Element> {
    fn get_attribute(self, namespace: Atom, local_name: &str) -> Option<Temporary<Attr>> {
    ...
    fn set_attribute(self, name: &str, value: AttrValue) {
        ...
        self.do_set_attribute(name.clone(), value, name.clone(),
            satom!(""), None, |attr| *attr.local_name() == name);
    }
    ...
    fn has_class(&self, name: &str) -> bool {
        self.get_attribute(satom!(""), "class").root().map(|attr| {
    ...
    fn has_attribute(self, name: &str) -> bool {
        ...
        self.deref().attrs.borrow().iter().map(|attr| attr.root()).any(|attr| {
            *attr.local_name() == name && attr.namespace == satom!("")
        })
    }
    ...
    fn set_bool_attribute(self, name: &str, value: bool) {
        ...
        if value {
            ...
            self.remove_attribute(satom!(""), name);
        }
    }
    ...
    fn get_string_attribute(self, name: &str) -> DOMString {
        assert!(name == name.to_ascii_lower().as_slice());
        match self.get_attribute(satom!(""), name) {
            Some(x) => {
                let x = x.root();
                x.deref().Value()
            }
    ...
    fn get_uint_attribute(self, name: &str) -> u32 {
        assert!(name == name.to_ascii_lower().as_slice());
        let attribute = self.get_attribute(satom!(""), name).root();
        match attribute {
            Some(attribute) => {
...
impl<'a> ElementMethods for JSRef<'a, Element> {
    ...
    fn GetNamespaceURI(self) -> Option<DOMString> {
        match self.namespace {
            satom!("") => None,
            ref ns => Some(ns.as_slice().to_string())
        }
    }
    ...
    fn GetAttributeNS(self,
        ...
        let namespace = Atom::from_slice(null_str_as_empty_ref(&namespace));
    ...
    fn SetAttribute(self,
        ...
        let value = self.parse_attribute(&satom!(""), &name, value);
        self.do_set_attribute(name.clone(), value, name.clone(), satom!(""), None, |attr| {
    ...
    fn SetAttributeNS(self,
        ...
        // Step 1.
        let namespace = Atom::from_slice(null_str_as_empty_ref(&namespace_url));
        ...
        match prefix {
            Some(ref prefix_str) => {
                ...
                // Step 6.
                if "xml" == prefix_str.as_slice() && namespace != sns!(XML) {
                    return Err(NamespaceError);
                }

                // Step 7b.
                if "xmlns" == prefix_str.as_slice() && namespace != sns!(XMLNS) {
                    return Err(NamespaceError);
                }
            },
            None => {}
        ...
        // Step 7a.
        if xmlns == name && namespace != sns!(XMLNS) {
            return Err(NamespaceError);
        }

        // Step 8.
        if namespace == sns!(XMLNS) && xmlns != name && Some("xmlns") != prefix {
            return Err(NamespaceError);
        }
    ...
    fn RemoveAttribute(self, name: DOMString) {
        ...
        self.remove_attribute(satom!(""), name.as_slice())
    }
...
impl<'a> VirtualMethods for JSRef<'a, Element> {
    ...
    fn bind_to_tree(&self, tree_in_doc: bool) {
        ...
        match self.get_attribute(satom!(""), "id").root() {
            Some(attr) => {
                let doc = document_from_node(*self).root();
...
impl<'a> style::TElement for JSRef<'a, Element> {
    ...
    fn get_link(&self) -> Option<&'static str> {
        ...
        match node.type_id() {
            ...
            ElementNodeTypeId(HTMLLinkElementTypeId) => self.get_attr(&satom!(""), "href"),
            _ => None,
         }
    }
    fn get_local_name<'a>(&'a self) -> &'a Atom {
    ...
    fn get_id<'a>(&self) -> Option<Atom> {
        self.get_attribute(satom!(""), "id").map(|attr| {
            let attr = attr.root();
            match *attr.value() {
